...
class TestRandomNames(unittest.TestCase):
    ...
    def test_get_name_no_arguments(self):
        result = randname.get_name("first_names")
        self.assertIsInstance(result, str)
    
    def test_get_name_year(self):
        year = 1990
        result = randname.get_name("first_names", year=year)
        self.assertIsInstance(result, str)
    ...
    def test_get_name_country(self):
        country = "US"
        result = randname.get_name("first_names", country=country)
        self.assertIsInstance(result, str)

    def test_get_name_weights(self):
        weights = False
        result = randname.get_name("first_names", weights=weights)
        self.assertIsInstance(result, str)

    def test_get_name_invalid_sex(self):
        # available_sex = ["M", "F"]
        sex = "D"
        with self.assertRaises(randname.InvalidSexArgument): randname.get_name("first_names", sex=sex)
